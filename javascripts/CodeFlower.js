...
CodeFlower.prototype.flatten = function(root) {
  ...
  function recurse_comment(node) {
    if (node.children) {
      node.comment = node.children.reduce(function(p, v) {
        return p + recurse_comment(v);
      }, 0);
    }
    if (!node.id) node.id = ++i;
    nodes.push(node);
  ...
  root.comment = recurse_comment(root);
...
CodeFlower.prototype.mouseover = function(d) {
  this.text.attr('transform', 'translate(' + d.x + ',' + (d.y - 5 - (d.children ? 3.5 : Math.sqrt(d.size) / 2)) + ')')
    .text(d.name + ": " + d.size + " loc & " + d.comment + " comments")
    .style('display', null);
};
